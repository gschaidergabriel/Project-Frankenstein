...
class MirrorAgent:
    ...
    def _run_session_inner(self):
        ...
        try:
            ...
            titan_text = (
                f"Entity session with {MIRROR_NAME} (Philosophical Mirror) "
            ...
            LOG.info("Titan ingest OK for %s session", MIRROR_NAME)
        except Exception as e:
        ...
        summary_msg = (
            f"[Entity Session] Gespräch mit {MIRROR_NAME} (Philosophical Mirror), "
            f"{elapsed_min} Minuten, {turn} Turns. "
            f"Themen: {topics_str_chat}. "
            f"Zusammenfassung: {summary}"
        )
        _write_chat_message("system", MIRROR_NAME, summary_msg, self.session_id)
        _short_summary = " ".join((summary or "").split()[:15])
        if _short_summary and not _short_summary.endswith("."):
            _short_summary += "."
        _notif_body = f"Philosophical session with Frank — {elapsed_min} minutes."
        if _short_summary:
            _notif_body += f"\n{_short_summary}"
        _write_overlay_notification(MIRROR_NAME, _notif_body, self.session_id)
