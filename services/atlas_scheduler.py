#!/usr/bin/env python3
"""
Atlas Scheduler — Idle-gated entry point for Atlas sessions.
===============================================================
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "atlas_scheduler.log", encoding="utf-8"),
        logging.StreamHandler(),
    ],
)
from config.logging_config import setup_file_logging
setup_file_logging("atlas")
...
def _check_pid_lock() -> bool:
    """Return True if NO Atlas session is running."""
    if PID_FILE.exists():
        try:
...
def main():
    LOG.info("Atlas scheduler triggered.")
    ...
    LOG.info("All gates passed. Starting Atlas session...")

    from ext.atlas_agent import run
    run()
