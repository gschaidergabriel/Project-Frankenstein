...
def _check_no_therapist() -> bool:
    """Return True if NO Dr. Hibbert session is running (prevent overlap)."""
    if THERAPIST_PID_FILE.exists():
        try:
            pid = int(THERAPIST_PID_FILE.read_text().strip())
            os.kill(pid, 0)
            LOG.info("Gate FAIL: Dr. Hibbert session running (PID %d)", pid)
            return False
        except (ProcessLookupError, ValueError):
            pass  # Stale PID
    return True
...
def main():
    ...
    gates = [
        ...
        ("no_therapist", _check_no_therapist),
        ("idle", _check_idle),
