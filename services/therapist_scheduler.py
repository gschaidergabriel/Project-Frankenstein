#!/usr/bin/env python3
"""
Therapist Scheduler — Idle-gated entry point for Dr. Hibbert sessions.
=======================================================================

Called by systemd timer 3x/day (09:00, 15:00, 21:00 ± 30min jitter).
Runs gate checks before launching a session. If any gate fails, exits silently.

Gates:
1. PID lock not held (no concurrent session)
2. xprintidle >= 300s (5 min idle)
3. Last user-Frank chat >= 300s ago
4. Not gaming
5. GPU load < 30%
"""
...
logging.basicConfig(
    ...
    handlers=[
        logging.FileHandler(LOG_DIR / "therapist_scheduler.log", encoding="utf-8"),
...
def _check_pid_lock() -> bool:
    """Return True if NO session is running."""
    if PID_FILE.exists():
        try:
...
def main():
    LOG.info("Therapist scheduler triggered.")
    ...
    LOG.info("All gates passed. Starting Dr. Hibbert session...")
