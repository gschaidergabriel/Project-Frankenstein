...
Training: Dream consolidation phase (~1×/day), <100ms per cycle.
...
try:
    from config.paths import AICORE_DATA, DB_PATHS
except ImportError:
    AICORE_DATA = Path.home() / ".local" / "share" / "frank"
    DB_PATHS = {"titan": AICORE_DATA / "db" / "titan.db"}

LOG = logging.getLogger("titan.cortex")

MODEL_PATH = AICORE_DATA / "models" / "titan_cortex.pt"
...
# Cold start thresholds — modules return defaults until enough data
_COLD_MIS = 50       # 50 access log entries
_COLD_ET = 100       # 100 mood-labeled ingestions
_COLD_RWL = 50       # 50 feedback entries
_COLD_AS = 100       # 100 co-retrieval records
_COLD_CG = 200       # 200 access records
_COLD_ID = 50        # 50 labeled contradiction pairs
...
if HAS_TORCH:
    ...
    class RetrievalWeightLearner(nn.Module):
        """~200 params. Learns retrieval score weights (replaces [0.4,0.3,0.2,0.1])."""
        def __init__(self):
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(8, 16), nn.GELU(),
                nn.Linear(16, 4),  # 4 weights, softmax applied externally
            )
    ...
    class AssociativeStrengthener(nn.Module):
        ...
        def __init__(self):
            ...
            self._scale = 0.1  # Output in [-0.1, 0.1]

        def forward(self, x: torch.Tensor) -> torch.Tensor:
            return self.net(x) * self._scale

    class ConsolidationGate(nn.Module):
        """~13K params. Decides keep/compress/forget during dream consolidation."""
        def __init__(self):
            super().__init__()
            self.net = nn.Sequential(
                nn.Linear(389, 32), nn.GELU(),
                nn.Linear(32, 16), nn.GELU(),
                nn.Linear(16, 3),  # softmax applied externally
            )
...
class TitanCortex:
    ...
    def __init__(self, db_path: Optional[Path] = None):
        ...
        self._db_path = db_path or DB_PATHS.get("titan", AICORE_DATA / "db" / "titan.db")
        self._model_path = MODEL_PATH
        self._lock = threading.RLock()
        self._db_local = threading.local()
        self._db_conns: List[sqlite3.Connection] = []  # track all thread-local connections
    ...
    def _cold_blend(self, neural_val, default_val, steps: int, threshold: int):
        """Blend neural output toward default during cold start."""
        if steps >= threshold:
            return neural_val
        rate = steps / threshold  # 0.0 → 1.0
    ...
    # ------------------------------------------------------------------
    # Inference methods (called from Titan hooks)
    # ------------------------------------------------------------------
    ...
    def score_importance(self, features: dict) -> float:
        ...
        features: origin(str), claim_count(int), entity_count(int),
                  topic_count(int), text_length(int), has_user(bool),
                  has_entity(bool), hour(int)
        """
    ...
    def tag_emotion(self, embedding: np.ndarray) -> Tuple[float, float]:
        ...
        v_out = self._cold_blend(neural_v, default_valence,
                                 self._et_steps, _COLD_ET)
        a_out = self._cold_blend(neural_a, default_arousal,
                                 self._et_steps, _COLD_ET)
        return v_out, a_out
    ...
    def get_retrieval_weights(self, features: dict) -> List[float]:
        ...
        features: rrf(float), conf(float), recency(float), graph(float),
                  query_len(int), n_results(int), valence(float), arousal(float)
        """
    ...
    def predict_edge_delta(self, src_emb: np.ndarray, dst_emb: np.ndarray,
        ...
        edge_features: confidence(float), age_days(float),
                       co_retrieval_count(int), same_origin(bool),
                       relation(str)
        """
    ...
    def gate_memory(self, embedding: np.ndarray, age_days: float,
        ...
        Returns: "keep", "compress", or "forget"
        """
        ...
        if self._cg_steps < _COLD_CG:
            ...
            # Blend toward [1.0, 0.0, 0.0] (always keep)
    ...
    @torch.no_grad()
    def detect_interference(self, emb_a: np.ndarray,
                            emb_b: np.ndarray) -> float:
        """ID: Detect if two memories contradict each other.
        Returns: interference score ∈ [0, 1] (0=compatible, 1=contradicts)
        """
    ...
    def _build_id_input(self, emb_a: np.ndarray,
                        emb_b: np.ndarray) -> torch.Tensor:
        """Build 768-dim input for ID."""
    ...
    def record_rwl_feedback(self, weights_used: List[float],
        ...
        """Record a retrieval weight feedback entry for RWL training."""
        try:
            conn = self._get_db()
            conn.execute(
    ...
    def train_cycle(self):
        """Full training cycle on accumulated data. Called during dream consolidation."""
        if not HAS_TORCH:
            return
        ...
        with self._lock:
            ...
            try:
                ...
                self._set_eval_mode()  # Always restore eval mode
    ...
    def _get_optimizer(self, name: str, module: nn.Module,
        ...
        """Get or create optimizer for a module."""
    ...
    def _train_mis(self):
        """Train MIS on access log data (binary: retrieved within 7d or not)."""
        conn = self._get_db()
        try:
            # Get recent nodes with access info
            rows = conn.execute("""
        ...
        for row in rows:
            ...
            claims = meta.get("claim_count", 0)
            entities = meta.get("entity_count", 0)
            topics = meta.get("topic_count", 0)
            text_len = len(meta.get("full_text", ""))
            has_user = "user" in meta.get("full_text", "").lower()
            has_entity = meta.get("has_entity", False)
            hour = 12  # default

            features = {
                "origin": origin, "claim_count": claims,
                "entity_count": entities, "topic_count": topics,
                "text_length": text_len, "has_user": has_user,
                "has_entity": has_entity, "hour": hour
            }
            inputs.append(self._build_mis_input(features).squeeze(0))
            # Target: was it retrieved? scale to [0.1, 1.0]
    ...
    def _train_et(self):
        """Train ET on mood-correlated ingestion data (weak supervision)."""
        # For now, ET trains on the assumption that memories ingested during
        # positive mood have positive valence. This is weak supervision.
        conn = self._get_db()
        try:
            rows = conn.execute("""
                SELECT n.metadata FROM nodes n
                WHERE n.type = 'memory'
                  AND json_extract(n.metadata, '$.valence') IS NOT NULL
                ORDER BY RANDOM() LIMIT 64
            """).fetchall()
        except Exception:
            return
        ...
        # ET needs embeddings → too expensive to recompute in training.
        # Training deferred until embedding batching is available.
        # Steps only increment when actual gradient updates happen.

    def _train_rwl(self):
        """Train RWL on feedback data (REINFORCE-style)."""
        conn = self._get_db()
        try:
            rows = conn.execute(
        ...
        # Build target weights: for positive reward, use the weights that were
        # used during that retrieval. For negative/zero reward, push toward defaults.
        target_weights = []
        for row in rows:
            w = _safe_json(row[0])
        ...
        # Weight targets by reward: blend toward defaults for low/negative reward
        defaults = torch.tensor([_DEFAULT_RETRIEVAL_WEIGHTS], dtype=torch.float32)
        # Blend: high reward → trust recorded weights, low → fall back to defaults
        blend = torch.sigmoid(R).unsqueeze(1)  # (N, 1) in [0, 1]
        W_blended = blend * W_target + (1 - blend) * defaults

        opt = self._get_optimizer("rwl", self.rwl, lr=3e-4)
        self.rwl.train()
        weights = self.rwl(X)  # (N, 4) softmax output

        # Supervised: minimize distance to reward-blended target weights
    ...
    def _train_as(self):
        """Train AS on co-retrieval data (Hebbian learning)."""
        conn = self._get_db()
        try:
            rows = conn.execute(
                "SELECT node_a, node_b, count FROM co_retrieval "
                "ORDER BY count DESC LIMIT 32"
            ).fetchall()
        except Exception:
            return
        ...
        # AS needs embeddings from VectorStore — too expensive in training.
        # Steps only increment when actual gradient updates happen.

    def _train_cg(self):
        """Train CG on post-consolidation access patterns."""
        conn = self._get_db()
        try:
            # Find nodes that were marked for potential pruning but later accessed
            rows = conn.execute("""
                SELECT n.id, n.metadata,
                    (SELECT COUNT(*) FROM access_log a
                     WHERE a.node_id = n.id
                       AND a.timestamp > ?) as recent_access
                FROM nodes n
                WHERE n.type = 'memory'
                  AND n.protected = 0
                ORDER BY RANDOM() LIMIT 64
        ...
        # CG training requires embeddings → batched training deferred.
        # Steps only increment when actual gradient updates happen.

    def _train_id(self):
        """Train ID on existing contradiction labels."""
        conn = self._get_db()
        try:
            # Positive examples: existing contradicts edges
            pos = conn.execute(
                "SELECT src, dst FROM edges WHERE relation = 'contradicts' "
                "ORDER BY RANDOM() LIMIT 16"
            ).fetchall()
            # Negative examples: random non-contradicting pairs
            neg = conn.execute(
                "SELECT src, dst FROM edges WHERE relation != 'contradicts' "
                "ORDER BY RANDOM() LIMIT 16"
            ).fetchall()
        ...
        # ID training requires embeddings → deferred to when embeddings available.
        # Steps only increment when actual gradient updates happen.

    # ------------------------------------------------------------------
    # Access logging (called from retrieval hooks)
    # ------------------------------------------------------------------

    def log_access(self, node_ids: List[str], query: str,
                   scores: Optional[List[float]] = None):
        """Log which nodes were retrieved for a query."""
        if not node_ids:
            return
        try:
            conn = self._get_db()
    ...
    def log_co_retrieval(self, node_ids: List[str]):
        """Log co-retrieval pairs (nodes retrieved in same query)."""
        if len(node_ids) < 2:
            return
        try:
            conn = self._get_db()
    ...
    def _get_db(self) -> sqlite3.Connection:
        """Thread-local DB connection to titan.db."""
    ...
    def _init_tables(self):
        """Ensure cortex tables exist in titan.db."""
        try:
            conn = self._get_db()
    ...
    def _save_all(self):
        """Atomic save of all 6 models + training state."""
        try:
    ...
    def _load_all(self):
        """Load all 6 models from disk if checkpoint exists."""
    ...
    def get_stats(self) -> dict:
        """Return cortex stats for debugging."""
        conn = self._get_db()
        try:
    ...
    def close(self):
        """Close all DB connections (from all threads)."""
        with self._db_conns_lock:
...
def get_cortex(db_path: Optional[Path] = None) -> Optional[TitanCortex]:
    ...
    if _cortex is None:
        with _cortex_lock:
            if _cortex is None:
                try:
                    _cortex = TitanCortex(db_path=db_path)
