...
class CommandRouterMixin:
    ...
    def _on_send(self):
        ...
        try:
            ...
            try:
                ...
                self._chat_q.put(("chat", {"msg": msg, "max_tokens": DEFAULT_MAX_TOKENS, "timeout_s": DEFAULT_TIMEOUT_S, "task": "chat.fast", "force": None}))
    ...
    def _route_message(self, msg: str):
        ...
        if DARKNET_RE.search(low):
            # Extract query: strip darknet/tor/onion keywords
            q = re.sub(
                r"(such\w*\s+(?:im\s+)?(?:darknet|dark\s*web|tor\s*netz|onion|hidden\s*service)\s*"
                r"|darknet\s*such\w*\s*|dark\s*web\s*search\s*|tor\s*search\s*|onion\s*such\w*\s*"
                r"|nach\s+)",
